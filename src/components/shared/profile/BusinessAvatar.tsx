...
const BusinessAvatar: FunctionComponent<IProfileProps> = ({
  handleProfileList,
  business,
}) => {
...
return (
    ...
    >
      <div className="avatar_info hidden sm:block">
        <div className="avatar_info-business_name">{business?.name}</div>
      ...
      <div className="avatar-icon flex items-center">
        {business?.file?.url ? (
          <span className='h-[50px] min-w-[50px] w-[50px]'>
            <img
              src={business.file.url}
              alt="Business Avatar"
          ...
          <Avatar className="h-[50px] w-[50px] rounded-full p-1 bg-[#EF8700] flex items-center justify-center">
            <AvatarImage
              ...
              className="bg-transparent text-white h-4    w-4"
            />
          </Avatar>
        )}
